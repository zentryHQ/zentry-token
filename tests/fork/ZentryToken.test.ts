...
describe("ZentryToken", () => {
	...
	it("set deployed parameter correctly", async () => {
		...
		expect((await migrator.zentryToken()).toLowerCase()).to.equal((zentryToken.target as string).toLowerCase())
	...
	it("grant MINT_ROLE to migrator", async () => {
		...
		expect(await zentryToken.hasRole(MINTER_ROLE, migrator.target)).to.equal(true)
	})
	...
	it("renounce ZentryToken ADMIN_ROLE from deployer", async () => {
		...
		expect(await zentryToken.hasRole(DEFAULT_ADMIN_ROLE, deployer.address)).to.equal(false)
	})

	it("transfer Migrator ownership to owner", async () => {
		const { migrator } = await loadFixture(deployTokenFixture)
		expect((await migrator.owner()).toLowerCase()).to.equal(owner.toLowerCase())
	})

	it("should migrate GF to ZentryToken", async () => {
		const { zentryToken, migrator } = await loadFixture(deployTokenFixture)
		const whale = await impersonate("0xd55CAde5F46B3B7E20eC0B30e174FD7013FD6170")
		...
		console.log("GF Balance Before ", ethers.formatEther(gfBalanceBefore.toString()).toString())
		console.log("ZentryToken Before ", ethers.formatEther(zentryBalanceBefore.toString()).toString())
		...
		console.log("GF Balance After ", ethers.formatEther(gfBalanceAfter.toString()).toString())
		console.log("ZentryToken After ", ethers.formatEther(zentryBalanceAfter.toString()).toString())
	...
	it("can transfer zentry token", async () => {
		...
		const _gfToken = await ethers.getContractAt("IERC20", await zentryToken.getAddress(), whale)
		...
		expect(zentryBalanceAfterTransfer.toString()).to.equal((zentryBalanceAfter + zentryBalanceBefore).toString())
	})
})
